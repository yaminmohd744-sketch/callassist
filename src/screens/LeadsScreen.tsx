...
function parseCSV(text: string): Omit<Lead, 'id' | 'callCount' | 'createdAt' | 'lastCalledAt'>[] {
  ...
  return lines.slice(1, MAX_CSV_IMPORT + 1).map(line => {
...
export function LeadsScreen({ userId, onCallLead, pastSessions }: LeadsScreenProps) {
  ...
  const leadSessions = useMemo(() => {
    ...
    const name    = selectedLead.name.trim().toLowerCase();
    const company = (selectedLead.company ?? '').trim().toLowerCase();
    return pastSessions
      .filter(s => {
        const pn = (s.config.prospectName ?? '').trim().toLowerCase();
        const pc = (s.config.company ?? '').trim().toLowerCase();
  ...
  async function handleImport(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const truncated = parsed.length === MAX_CSV_IMPORT;
  ...
  const pkgLabel = activePackage?.name ?? 'All Leads';

  // Single hidden file input shared across packages + leads views
  return (
    <>
        ...
        <div className="leads-screen" key="packages">
          <div className="leads-screen__header">
            <div className="leads-screen__header-left">
              ...
              {/* +1 counts the always-present "All Leads" entry */}
        ...
        <div className="leads-screen" key="leads">
          <div className="leads-screen__header">
            <div className="leads-screen__header-left">
              ...
              <h1 className="leads-screen__title">{pkgLabel.toUpperCase()}</h1>
          ...
          {loading ? (
            ...
            <div className="leads-screen__empty">
              ...
              <p className="leads-screen__empty-text">
                Import a CSV or add leads manually — they'll be automatically assigned to <strong>{pkgLabel}</strong>.
              </p>
              ...
              {leadSessions.length === 0 ? (
                ...
                <div className="leads-screen__history-list">
                  {leadSessions.map((session, i) => {
                    ...
                    return (
                      <div key={session.id ?? i} className="leads-screen__history-card">
                        <div className="leads-screen__history-card-left">
                          <div className="leads-screen__history-date">{formatDateShort(session.endedAt, appLanguage)}</div>
