...
if (!url || !key) {
  ...
  console.error('[Pitchr] Missing VITE_SUPABASE_URL or VITE_SUPABASE_ANON_KEY — set these in your deployment environment variables');
  // In dev, throw immediately so misconfiguration is obvious rather than manifesting as mysterious auth failures.
  if (import.meta.env.DEV) throw new Error('[Pitchr] Missing Supabase env vars — add VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY to .env.local');
}
